...
output "configuration_guide" {
  description = "Next steps to configure the load balancer"
  value = <<-EOT
    ...
    1. SSH into Nginx server: 
       ssh -i ~/.ssh/id_ed25519 ec2-user@${module.nginx_server.public_ip}
    ...
    ${join("\n    ", [for name, server in module.backend_servers : "- ${name}: Public: ${server.public_ip} | Private: ${server.private_ip}"])}
